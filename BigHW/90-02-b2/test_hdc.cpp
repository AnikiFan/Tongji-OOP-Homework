...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
static void to_be_continued(const char* prompt_prefix, const char* name, const char* prompt_suffix, const int X = 0, const int Y = 22)
{
	//先输出一行空格，为了清空上次字符串比本次长而多出来的后续部分
	cct_gotoxy(X, Y);
	cout << "                                                                                          " << endl;
	...
	cct_gotoxy(X, Y);
	if (prompt_prefix)
		cout << prompt_prefix << name << prompt_suffix << "，按回车键继续...";
	else
		cout << "按回车键继续...";
...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_LINE] == '1') {
		...
		/* 横线(双向) */
		hdc_line(100, 100, 500, 100, 10, RGB(rand() % 256, rand() % 256, rand() % 256));
	...
	if (test_choise[T_ELLIPSE] == '1') {
		...
		for (round = 0; round <= 1; round++)
			for (i = 0; i < 360; i += 15) {
				...
				to_be_continued(tmp, 0, 0);
				hdc_ellipse(350, 350, 300 - round * 20, 200 - round * 20, i, round, 20 + rand() % 5, color[i / 15 % 3]);	//预置三色轮换
			}
	...
	if (test_choise[T_PEPPAPIG] == '1') {
		...
		to_be_continued("小猪佩奇显示完成", STL_X, STL_Y);
	}

	if (test_choise[T_MATHCURVE] == '1') {
		/* 画一条数学曲线 */

		/* 无名结构体的定义 + 指向函数的指针的使用，送给大家了 */
		struct {
			int sno;
			const char* name;
			void (*fun)(void);
		} hdc_draw_Mathematical_curve[] = {
			{2254298, "范潇", hdc_draw_Mathematical_curve_2150244},
			{0,NULL,NULL}
		};
		
		/* 显示显示助教哥哥/姐姐们画的数学曲线
		   注：调用函数前，先要添加对应的cpp并在.h中加函数声明
		*/
		for (i = 0; hdc_draw_Mathematical_curve[i].sno > 0; i++) {
			hdc_draw_Mathematical_curve[i].fun();
			to_be_continued("数学曲线-", hdc_draw_Mathematical_curve[i] .name, "-完成", STL_X, STL_Y);
		}
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		/* 无名结构体的定义 + 指向函数的指针的使用，送给大家了
		   可以将上下两个结构体合并在一起，即无名struct中含两个函数指针，此处为了方便，分开写 */
		struct {
			int sno;
			const char* name;
			void (*fun)(const int base_x, const int base_y);
			int x;
			int y;
		} hdc_draw_cartoon[] = {
		...
		/* 显示显示助教哥哥/姐姐们画的卡通图像
		   注：调用函数前，先要添加对应的cpp并在.h中加函数声明
		*/
		for (i = 0; hdc_draw_cartoon[i].sno > 0; i++) {
			hdc_draw_cartoon[i].fun(hdc_draw_cartoon[i].x, hdc_draw_cartoon[i].y); //为了统一，此处的参照点坐标定死为（400,400）
			to_be_continued("卡通图像-", hdc_draw_cartoon[i].name, "-完成", STL_X, STL_Y);
		}
